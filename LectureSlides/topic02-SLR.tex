...
% 	\frametitle{ }
% 	\begin{itemize}
%    \item Which one of the above do you think has \textbf{highest correlation} and ideal for linear regression?
% 			\pause
% 	\vspace{0.5cm}
%
% 	\item In \textbf{all the four graphs}: mean of x = 9 (with variance 11); mean of y = 7.50 (with variance 4.1) ; correlation between x and y = 0.816
% 	\vspace{0.5cm}
%
% 	\item Fitted linear regression in each cases: \textbf{$y = 3 + 0.5x $}
% 	\vspace{0.5cm}
%
% 	\item In 1973, \textbf{Anscombe} demonstrated the \textbf{importance} of \textbf{graphing data}
%       %\footnote{The graph is taken from the Wikipedia}
%       before analyzing it and the \textbf{effect of outliers} on statistical properties
% \end{itemize}	
% 	
% \end{frame}
...
\begin{frame}
   ...
   \begin{theorem}
      Under the three assumptions (on slide 11), the LSEs $\hat\beta_0$ and
...
\vspace{0.2cm}
		\item  $R^2$ is interpreted as the \textbf{``percentage of variation explained by the model''.}
...
\item In \textbf{multiple linear regression}, \textbf{adding a variable to a model can increase the value of $R^{2}$}. To overcome this problem:

\[ 
\mbox{Adjusted } R^{2}: \hspace{0.5cm}  R^{2}_{adj}  = 1 - \frac{\sum_{i}(y_{i} - \hat y_{i})^{2}/(n-p-1)}{\sum_{i}(y_{i} - \bar y)^{2}/(n-1)}.
\]
\end{itemize}	
...
\begin{itemize}
	\item Data on \textbf{heights of $n=1375$ mothers} in the UK under the age of 65 and one of their \textbf{adult daughters over the age of 18} (collected and organized during the period 1893--1898 by the famous statistician Karl Pearson).
	\bigskip
	\item A historical \textbf{use of regression to study inheritance of height from generation to generation.}
	\bigskip
	%		\item Let's try R to play with this data set.
\end{itemize}		
\end{frame}
...
\begin{frame}
	...
	\begin{figure}
		\begin{center}
			\scalebox{0.4}[0.4]{\includegraphics{scatterplot1.eps}}
		\end{center}
	\end{figure}
\end{frame}
...
\begin{frame}
	\begin{figure}
		\begin{center}
			\scalebox{0.4}[0.4]{\includegraphics{regline1.eps}}
		\end{center}
	\end{figure}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item It's an empirical phenomenon, also called \textcolor{red}{``regression to the mean'' or ``regression to mediocrity'',} e.g.
		\smallskip
		\begin{itemize}
			\item Daughters of tall mothers \textcolor{red}{tend to be tall, but not as tall} as their mothers; daughters of short mothers tend to be short, but not as short as their mothers (same trend between sons and fathers)
			\bigskip
			\item Students \textcolor{red}{doing well in Mid-term tend to do well in the Final, but not as well} as the Mid-term; students doing poorly in Mid-term tend to do poorly in the Final, but not as poorly as the Mid-term!
		\end{itemize}
	\end{itemize}
\end{frame}
...
\vspace{0.2cm}			
			\item What type of the \textbf{study design used}: \textit{Randomized or Observational; Prospective or Retrospective etc. }
\vspace{0.2cm}						
			\item Can you make \textbf{connection with the primary research question and the data}? Is the \textbf{research question feasible} based on the data you have?
			\vspace{0.2cm}			
			\item Are there \textbf{secondary research questions}?
			\vspace{0.2cm}			
			\item  What are the \textbf{potential source of bias}? Sample (data) \textbf{may not be} a representative of the target (source) population.
			\vspace{0.2cm}			
			\item Are there any \textbf{confounders}? 
			\vspace{0.2cm}			
			\textbf{Confounders}: A confounder (also known as confounding variable, confounding factor) is \textbf{a variable that influences both the dependent variable and independent variable}, causing a spurious association. \textbf{Confounding is a causal concept}, and as such, cannot be described in terms of correlations or associations.
			\vspace{0.2cm}			
			\item  Are the \textbf{number of observations/individuals} in the data \textbf{sufficient}? Is it a \textbf{small data}?
		\end{enumerate}
...
\item \textbf{Interpret} the output from the fitted models:
			\vspace{0.2cm}
\begin{enumerate}
	\item Are all the results expected? Whether the \textbf{results go well with existing domain} (basic science) knowledge?
			\vspace{0.2cm}	
	\item If not, what are the \textbf{reasons behind the aberration} from the expected results.  
\end{enumerate}

\item \textbf{Check the diagnostics} for model assumptions. If you find problem, go back and correct (if you can) the chosen model; or, take decision about the outliers/influential points. 
			\vspace{0.2cm}
\item Always remember: 	\emph{\textcolor{blue}{``Essentially, all models are wrong, but some are useful'' -- Box}}
	\end{itemize}	
...
% 	
% 	\begin{itemize}
% 		\item A researcher wants to see \textbf{whether the occupation `bartender' is a source of lung cancer}. Is the research question correct to you?
% 			\vspace{0.5cm}		
% 		\item  Does \textbf{Caesarean section (C-section) increase} the chance of \textbf{childhood asthma} in a prospective study in Delhi/Beijing? 
% 	\end{itemize}
% 		
% \end{frame}
%
...
% %	\frametitle{ }
% 	\begin{center}
% 	\includegraphics[trim=2cm 0cm 2cm 0cm, scale=0.4]{Choco_Nobel_prize_NEJM.jpg}	
% \end{center}	
% 	
% \end{frame}
%
...
%
% \begin{itemize}
% 	\item The \textbf{New England Journal of Medicine (NEJM)} is the most \textbf{prestigious journal of medical science}. The impact factor of NEJM is 176.1 (2021)!
% 		\vspace{0.5cm}	
% 	\item \textbf{Example: Bad science or fraud science??}
% 		\vspace{0.5cm}	
% 	\item The above article\footnote{Franz, H. Messerli (2012). Chocolate Consumption, Cognitive Function, and Nobel Laureates. The New England Journal of Medicine, 367, 16.} is an example of \textbf{spurious correlation}. 
% 		\vspace{0.5cm}	
% 	\item My intuition: if you \textbf{replace the x-axis with per capita cows} you may see the \textbf{same pattern!} Since most of the chocolate producing Europian countries have many per capita cows. 
% \end{itemize}
%
%
%
...
%
% \begin{frame}
% %	\frametitle{\textbf{Different types of Studies\footnote{Grimes, D. A., \& Schulz, K. F. (2002). An overview of clinical research: the lay of the land. The Lancet, 359(9300), 57-61.}: } }
% 	
% \begin{multicols}{2}	
% \begin{center}
% 	\includegraphics[scale=0.8]{Study_design_Grimies_lancet_2002.jpg}	
% \end{center}
%
% \begin{itemize}
% 	\item \textbf{Different types of Studies}
% 	\vspace{0.5cm}
% 	\item The approach, interpretation, issue of bias and role of confounding factor will be different depending upon the study-design that generates the data. 
% 		\vspace{0.5cm}
% 	\item Reference: Grimes, D. A., \& Schulz, K. F. (2002). An overview of clinical research: the lay of the land. The Lancet, 359(9300), 57-61.
% \end{itemize}	
% \end{multicols}
% 	
% \end{frame}
