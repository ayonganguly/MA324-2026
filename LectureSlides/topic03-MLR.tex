...
\begin{frame}
   ...
   \begin{itemize}
      \item Variance Inflation Factors (VIFs):
         \begin{itemize}
            \item It can be shown that
               $$Var(\hat{\beta_j}) = \sigma^2 C_{jj}=\frac{\sigma^2}{1-R_{j}^2},$$
...
\begin{frame}{Singular Value Decomposition (SVD)}
    ...
    \begin{itemize}
        ...
        \item $\mathbf{\Gamma}$ is a $p \times p$ diagonal matrix of
           \textbf{singular values} $\mu_1, \mu_2, \dots, \mu_p$.
    \end{itemize}
    \vspace{0.2cm}
    Note: The eigenvalues of $\mathbf{X}^T\mathbf{X}$ are $\lambda_j = \mu_j^2$.
\end{frame}

\begin{frame}{Condition Number and Condition Indices}
    The \textbf{Condition Number} of the matrix $\mathbf{X}$ is defined as:
    $$\eta = \frac{\mu_{\max}}{\mu_{\min}}$$
...
\begin{frame}{Variance Decomposition Proportions}
    ...
    $$\text{Var}(\hat{\beta}_j) = \sigma^2 \sum_{k=1}^{p} \frac{v_{jk}^2}{\mu_k^2}$$
    ...
    $$\pi_{kj} = \frac{v_{jk}^2 / \mu_k^2}{\sum_{i=1}^{p} v_{ji}^2 / \mu_i^2}$$
    \begin{itemize}
